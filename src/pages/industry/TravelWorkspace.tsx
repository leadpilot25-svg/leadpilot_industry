...
function EmptyRow({ cols, message }: { cols: number; message: string }) {
  return (
    <tr><td colSpan={cols} className="px-4 py-10 text-center text-sm text-gray-400">{message}</td></tr>
  )
}
...
function GuidanceCard() {
  return (
    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-center space-y-3">
      ...
      <div>
        <p className="text-sm font-semibold text-amber-900">Track departures and bookings</p>
        <p className="text-xs text-amber-700 mt-1 leading-relaxed">Add departure dates and booking details to your leads to see upcoming trips and visa tracking.</p>
      </div>
      <div className="rounded-xl border border-amber-200 bg-white px-3 py-2">
        <p className="text-[10px] font-semibold text-amber-600 uppercase tracking-wide mb-1">Custom fields to fill</p>
        <p className="font-mono text-[11px] text-gray-600">departure_date, booking_status, destination</p>
      </div>
  ...
  return (
    <AppLayout>
      <div className="px-4 py-6 sm:px-6 space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-xl font-semibold text-gray-900">My Business — Travel</h1>
          ...
          <div className="flex items-center gap-2 border-b border-gray-100 px-5 py-3">
            ...
            {(['30','60','90'] as const).map(d => (
              <button key={d}
                ...
                className={`rounded-lg px-3 py-1 text-xs font-medium transition ${daysFilter === d ? 'bg-emerald-600 text-white' : 'border border-gray-200 text-gray-500 hover:text-gray-800'}`}
              >{d} days</button>
            ))}
          </div>
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {!loading && upcoming.map(lead => (
                  <LeadRow key={lead.id} lead={lead} onClick={() => navigate(`/leads/${lead.id}`)}>
                    <td className="px-4 py-3 text-sm text-gray-700">{cd(lead, 'destination') || '—'}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{formatDate(cd(lead, 'departure_date'))}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{formatDate(cd(lead, 'return_date'))}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{cd(lead, 'passengers') || '—'}</td>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {!loading && passportExp.map(lead => {
                  ...
                  return (
                    <LeadRow key={lead.id} lead={lead} onClick={() => navigate(`/leads/${lead.id}`)}>
                      <td className="px-4 py-3 text-sm text-gray-700">{cd(lead, 'passport_number') || '—'}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{formatDate(cd(lead, 'passport_expiry'))}</td>
                      <td className="px-4 py-3">
