...
import { LeadFormFields, type LeadFormData } from '../../components/leads/LeadFormFields'
...
function leadToForm(lead: Lead): LeadFormData {
  return {
    ...
    followup_date:     lead.followup_date
      ? new Date(lead.followup_date).toISOString().slice(0, 16)
      : '',
  }
}
...
export function EditLeadPage() {
  ...
  useEffect(() => {
    if (!id) return
    fetchLeadById(id)
      .then(l => { setLead(l); setForm(leadToForm(l)); setCustomData((l.custom_data as Record<string, unknown>) ?? {}) })
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault()
    if (!id || !form || !form.name.trim()) return
